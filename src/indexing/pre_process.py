...
def _context_compact(ctx: PageContext) -> str:
    parts = [ctx.section, ctx.subsection, ctx.subsubsection]
    return " - ".join([p for p in parts if p])


def _convert_detected_lines(lines: list[str]) -> str:
...
def _truncate_summary_words(summary: str, max_words: int = 20) -> str:
...
def _make_chunk(
    ...
    return {
        ...
        "has_table_rows": table_row_count > 0,
...
def _extract_intro_summary(lines: list[str]) -> str | None:
    for line in lines:
...
def _build_table_chunk_summary(lines: list[str], labels: list[LineLabel], page_ctx: PageContext) -> str:
...
def _refresh_chunk(chunk: dict, recompute_summary: bool = False) -> None:
    ...
    chunk["has_table_rows"] = table_row_count > 0
...
def _merge_small_chunks_to_previous(chunks: list[dict], max_small_chars: int) -> list[dict]:
    if not chunks:
        return chunks
...
def _split_large_chunks_by_chars(chunks: list[dict], max_chunk_chars: int) -> list[dict]:
    ...
    for chunk in chunks:
        ...
        parts_labels: list[list[LineLabel]] = []
        cur_lines: list[str] = []
        cur_labels: list[LineLabel] = []
        cur_chars = 0
...
def _apply_char_overlap(chunks: list[dict], overlap_chars: int) -> None:
    if overlap_chars <= 0:
        return
