...
class TableAwareRetriever:
    ...
    def _rank_hybrid_embedding_pool(self, pool, bm25_query, embedding_query, top_k, table_bonus=True):
        if not pool:
            return []
    ...
    def _rank_bm25_only(self, pool, bm25_query, top_k):
        if not pool:
            return []
        bm25_raw = self._bm25_score_map(pool, bm25_query, text_field="text")
        bm25_norm = self._normalize_score_map(bm25_raw)
        ranked_idx = sorted(bm25_norm.keys(), key=lambda i: bm25_norm.get(i, 0.0), reverse=True)[:top_k]
        ranked = []
        for final_rank, idx in enumerate(ranked_idx, start=1):
            item = dict(pool[idx])
            item["bm25_score"] = round(bm25_norm.get(idx, 0.0), 6)
    ...
    def _rank_embedding_only(self, pool, embedding_query, top_k):
        if not pool:
            return []
        emb_similarity_raw = self._embedding_similarity_map(pool, embedding_query, text_field="hybrid_embedding_text")
        emb_norm = self._normalize_score_map(emb_similarity_raw)
        ranked_idx = sorted(emb_norm.keys(), key=lambda i: emb_norm.get(i, 0.0), reverse=True)[:top_k]
        ranked = []
        for final_rank, idx in enumerate(ranked_idx, start=1):
            item = dict(pool[idx])
            item["embedding_score"] = round(emb_norm.get(idx, 0.0), 6)
    ...
    def retrieve(
        ...
        match_method="hybrid",  # "hybrid", "bm25", or "embedding"
        table_bonus=True,  # whether to apply table bonus in hybrid ranking
    ):
        _ = variable_name
        ...
        if match_method == "bm25":
            ranked = self._rank_bm25_only(pool, bm25_q, top_k)
        elif match_method == "embedding":
            ranked = self._rank_embedding_only(pool, embedding_q, top_k)
        else:
            ranked = self._rank_hybrid_embedding_pool(pool, bm25_q, embedding_q, initial_k, table_bonus)
            ranked = ranked[:top_k]
        return ranked